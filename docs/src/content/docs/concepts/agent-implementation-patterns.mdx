...
<Aside type="note">
  YAAgents has zero dependency on Claude Code or any specific LLM CLI. This section
  documents an AmpathyMinds-internal choice; your agent service can use whatever
  reasoning pipeline fits your domain. The pattern is §2.3 (process-shell); the CLI
  is just one instance of that pattern.
</Aside>
