...
<Aside type="note">
  **Looking for the recommended starter?** See [store-go (Go)](/examples/store-go/) —
  the Go ecommerce product-recommendations example. `campaign-api-go` is preserved as
  an alternative example showing the same Profile shape applied to a martech domain.
</Aside>
