...
A developer runs `docker compose up` in `examples/campaign-api/` and exercises `created`, `clarification_required`, `validation_failed`, and `failed_dependency` through the gateway with route-RBAC enforced and correlation-id propagated; `yaagents-cli conformance-test` returns PASS; and `pip install yaagents-fastapi` / `npm install @aimpathyminds/yaagents-client` / `docker pull ghcr.io/ai-mpathyminds/yaagents-gateway:0.1.0` succeed from the public registries.
